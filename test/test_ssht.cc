...
static ssht::DataReaders CreateReaders(unsigned n, Tips tips) {
	ssht::DataReaders out;
	out.reserve(n);
...
TEST(SSHT, Build) {
	...
	ssht::DataReaders fake_input;
	ASSERT_EQ(ssht::BuildSet(fake_input, fake_output), ssht::BUILD_STATUS_BAD_INPUT);
	ASSERT_EQ(ssht::BuildDict(fake_input, fake_output), ssht::BUILD_STATUS_BAD_INPUT);
	ASSERT_EQ(ssht::BuildDictWithVariedValue(fake_input, fake_output), ssht::BUILD_STATUS_BAD_INPUT);

	auto emb_gen = CreateReaders<EmbeddingGenerator>(1, EmbeddingGenerator::MASK0);
	ASSERT_EQ(ssht::BuildSet(emb_gen, fake_output), ssht::BUILD_STATUS_OK);
	ASSERT_EQ(ssht::BuildDict(emb_gen, fake_output), ssht::BUILD_STATUS_OK);
	ASSERT_EQ(ssht::BuildDictWithVariedValue(emb_gen, fake_output), ssht::BUILD_STATUS_OK);

	auto var_gen = CreateReaders<VariedValueGenerator>(1, 5U);
	ASSERT_EQ(ssht::BuildDict(var_gen, fake_output), ssht::BUILD_STATUS_BAD_INPUT);
	ASSERT_EQ(ssht::BuildDictWithVariedValue(var_gen, fake_output), ssht::BUILD_STATUS_OK);
...
TEST(SSHT, KeySet) {
	const std::string filename = "keyset.ssht";
	{
		ssht::FileWriter output(filename.c_str());
		auto input = CreateReaders<EmbeddingGenerator>(2, EmbeddingGenerator::MASK0);
		ASSERT_EQ(ssht::BuildSet(input, output), ssht::BUILD_STATUS_OK);
	}
	ssht::Hashtable dict(filename);
	ASSERT_FALSE(!dict);
	ASSERT_EQ(dict.type(), ssht::Hashtable::KEY_SET);
...
TEST(SSHT, InlinedDict) {
	const std::string filename = "dict.ssht";
	{
		ssht::FileWriter output(filename.c_str());
		auto input = CreateReaders<EmbeddingGenerator>(2, EmbeddingGenerator::MASK0);
		ASSERT_EQ(ssht::BuildDict(input, output), ssht::BUILD_STATUS_OK);
	}
	ssht::Hashtable dict(filename);
	ASSERT_FALSE(!dict);
	ASSERT_EQ(dict.type(), ssht::Hashtable::KV_INLINE);
	ASSERT_EQ(dict.key_len(), sizeof(uint64_t));
...
TEST(SSHT, VariedDict) {
	const std::string filename = "var-dict.ssht";
	{
		ssht::FileWriter output(filename.c_str());
		auto input = CreateReaders<VariedValueGenerator>(2, 5U);
		ASSERT_EQ(ssht::BuildDictWithVariedValue(input, output), ssht::BUILD_STATUS_OK);
	}
	ssht::Hashtable dict(filename);
	ASSERT_FALSE(!dict);
	ASSERT_EQ(dict.type(), ssht::Hashtable::KV_SEPARATED);
	ASSERT_EQ(dict.key_len(), sizeof(uint64_t));
	ASSERT_EQ(dict.item(), PIECE*2);
...
TEST(SSHT, FetchWithPatch) {
	const std::string base_filename = "base.ssht";
	const std::string patch_filename = "patch.ssht";
	{
		ssht::FileWriter base_output(base_filename.c_str());
		auto base_input = CreateReaders<EmbeddingGenerator>(2, EmbeddingGenerator::MASK1);
		ASSERT_EQ(ssht::BuildDict(base_input, base_output), ssht::BUILD_STATUS_OK);
		ssht::FileWriter patch_output(patch_filename.c_str());
		auto patch_input = CreateReaders<EmbeddingGenerator>(1, EmbeddingGenerator::MASK0);
		ASSERT_EQ(ssht::BuildDict(patch_input, patch_output), ssht::BUILD_STATUS_OK);
	}

	ssht::Hashtable base(base_filename);
	ASSERT_FALSE(!base);
	ssht::Hashtable patch(patch_filename);
	ASSERT_FALSE(!patch);
...
TEST(SSHT, RebuildInlinedDict) {
	std::string filename = "dict-old.ssht";
	{
		ssht::FileWriter output(filename.c_str());
	...
	}
	{
		ssht::Hashtable dict(filename);
		ASSERT_FALSE(!dict);
		filename = "dict-new.ssht";
		ssht::FileWriter output(filename.c_str());
		auto input = CreateReaders<EmbeddingGenerator>(2, EmbeddingGenerator::MASK0);
		ASSERT_EQ(dict.derive(input, output), ssht::BUILD_STATUS_OK);
	}
...
TEST(SSHT, RebuildVariedDict) {
	std::string filename = "var-dict-old.ssht";
	{
		ssht::FileWriter output(filename.c_str());
